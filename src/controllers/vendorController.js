        ...
        if (vendor) {
            ...
            if (status === 'Active') {
                ...
                const reasonBlock = reason
                    ? `<br><br><div style="background:#fef2f2;border-left:4px solid #dc2626;padding:16px 20px;border-radius:6px;margin-top:12px;"><strong style="color:#dc2626;">Reason for Rejection:</strong><br><span style="color:#7f1d1d;">${reason}</span></div>`
                    : '';
...
<table role="presentation" border="0" cellpadding="0" cellspacing="0" width="600" style="background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 10px 15px -3px rgba(0,0,0,0.1);border:1px solid #e2e8f0;">
<tr><td style="background-color:${darkColor};padding:35px 30px;text-align:center;">
...
<tr><td style="padding:25px;text-align:center;border-bottom:2px solid ${color};">
<div style="display:inline-block;padding:8px 16px;border-radius:50px;background-color:${color};color:#ffffff;font-weight:700;font-size:14px;text-transform:uppercase;">
...
<p style="color:#64748b;font-size:12px;margin:0;line-height:1.5;">&copy; ${new Date().getFullYear()} Mehfil One. All rights reserved.<br>
You are receiving this email because you are a registered vendor.</p>
</td></tr>
</table></td></tr></table>
</body></html>`;
...
exports.upgradeVendorPlan = async (req, res) => {
    try {
        const { paymentId, orderId, amount, billingCycle } = req.body;
...
exports.renewVendorPlan = async (req, res) => {
    try {
        const { paymentId, orderId, amount, planName, billingCycle } = req.body;
        const startDate = new Date();
        const expiryDate = new Date();
